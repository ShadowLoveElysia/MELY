...
3. 导入 `.pmx`、`.pmd` 模型或包含模型与贴图的 ZIP 文件，根据需要再导入舞蹈动作或表情 `.vmd`。
4. 已导入的动作轨和表情轨会分别显示；未导入的轨道不会占位。独立调整各轨帧数，选定组合姿态后分别锁定所有已载入轨道。
...
- 将 `.vmd` 的骨骼动作与表情 Morph 分为独立轨道；仅显示实际已导入的轨道。
- 动作轨与表情轨可分别播放、暂停、拖动时间轴、逐帧调整和锁定；最终预览与生成会合成两轨当前帧。
- 支持空格键优先播放或暂停动作轨；未载入动作轨时控制表情轨。
- 使用 GPU 实时绘制动画，锁帧和体素化时再生成 CPU 端蒙皮快照。
- 手动调整模型骨骼，并导入或导出轻量级 MELY Pose JSON。
- 实体方块体素化和 Ethereal Hologram 全息线框生成模式。
- 使用竖直末地烛与独立白色玻璃板表现稀疏的人物轮廓，并可独立调节内部投影密度。
...
- 为 2032/4064 层扩展高度提供主动解锁、多阶段风险提示和可尝试的生成/导出流程。
- 中文、English、日本語界面。
...
1. 只需修改 `src/version/version.json` 中的版本号。桌面构建与 Windows 发布入口会
   自动同步 Cargo 所需的派生版本字段；如需立即刷新，可手动运行 `npm run version:sync`。
...
2. 根据需要导入舞蹈动作 VMD、表情 VMD，或同时包含骨骼与 Morph 的 VMD。
3. MELY 只显示实际载入的轨道；分别播放或拖动动作轨与表情轨，组合出合适的动作帧和表情帧。
4. 分别锁定所有已载入轨道，将当前组合姿态用于体素化和投影生成。
...
- Java 版本信息由精确 Profile 管理。所有列出的版本都允许以 best-effort 方式尝试生成与导出；Java 1.20.1 通过了仓库格式测试，其他未充分测试的版本可能回退到已知序列化器，并存在兼容问题或少量 Bug。仓库 NBT 回读不等于 Litematica/WorldEdit 真实工具验收。
- 默认推荐角色高度为 320 格，为原版世界上下边界预留安装空间。
- 默认世界范围由所选精确版本 Profile 给出；Java 1.20.1 为 `Y=-64..319`（384 层）。
- 2032 层扩展可由用户主动解锁并尝试生成、导出。目标世界通常仍需使用与精确版本、目标维度匹配的第三方数据包；默认声明为 `min_y=-1024, height=2032`，可按实际数据包修改。
- 4064 层实验状态可在解锁 2032 后继续开启，并通过解锁、环境提示和单次导出确认。参考配置为 `min_y=-2032, height=4064`（可放置 `Y=-2032..2031`），不能称为“原生 4096”。该流程尚未充分测试，可能存在兼容问题或少量 Bug。
- 将超限投影载入普通世界可能造成顶部截断、游戏卡顿、模组崩溃或存档风险。

MELY 不制作、捆绑、下载、安装、验证或担保第三方高度数据包。建议核对来源、许可证、精确版本、`pack_format`、`dimension_type`、`min_y` 和 `height`，并在世界副本中测试最低/最高边界、区块重载与实际粘贴工具；这些提示不会阻止生成或尝试导出，遇到兼容问题请反馈给社区。
