...
$outputRoot = Join-Path $projectRoot "release-validation\height-export-five-format"
$nodeCandidates = @(
  "C:\Program Files\nodejs\node.exe",
...
if ($DownloadTimeoutMs -lt 1000) { throw "DownloadTimeoutMs must be at least 1000." }
...
$env:MELY_E2E_OUTPUT = $outputRoot
...
$env:MELY_DOWNLOAD_TIMEOUT_MS = [string]$DownloadTimeoutMs
$env:MELY_E2E_ONLY_FORMAT = ""
$env:MELY_E2E_STOP_AFTER_SAFE = "0"
...
try {
  ...
  $audit = Start-Process -FilePath $node -ArgumentList @(
    "scripts\verify-height-export-e2e.cjs"
