...
$outputRoot = Join-Path $projectRoot "release-validation\survival-responsive-e2e"
$nodeCandidates = @(
  "C:\Program Files\nodejs\node.exe",
...
if ($TargetHeight -lt 32 -or $TargetHeight -gt 384) {
  throw "TargetHeight must be between 32 and 384."
}
if ($SupportBlockCount -le 0) {
  throw "SupportBlockCount must be positive."
}
...
$env:MELY_TARGET_HEIGHT = [string]$TargetHeight
$env:MELY_SUPPORT_BLOCK_COUNT = [string]$SupportBlockCount
...
try {
  ...
  $audit = Start-Process -FilePath $node -ArgumentList @(
    "scripts\verify-survival-responsive-e2e.cjs"
