...
const prepareGeneration = (
  ...
  if (document.edition !== "bedrock" || document.minecraftVersion !== DEFAULT_BEDROCK_VERSION.id) {
    throw new RangeError(
      `Bedrock function export requires a Bedrock ${DEFAULT_BEDROCK_VERSION.id} projection document`,
    );
  }
  assertProjectionDocumentIntegrity(document, "Bedrock function export");
  assertProjectionDocumentHologramIsolation(document, "Bedrock function export");
  ...
  const manifest: McfunctionManifest = {
    ...
    header: {
      ...
      uuid: headerUuid,
      version: [1, 0, 0],
      min_engine_version: [...DEFAULT_BEDROCK_VERSION.minEngineVersion],
    },
